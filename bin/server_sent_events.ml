type message_object = { i : int } [@@deriving yojson]

let server_state = ref []
let notify = ref ignore
let last_message = ref 0
...
let rec message_loop () =
  ...
  let message =
    { i = !last_message } |> yojson_of_message_object |> Yojson.Safe.to_string
  in
