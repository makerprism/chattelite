    ...
    schema
            ...
            ~resolve:(fun (info : resolve_info_t) () user_id display_name ->
              let* unit_or_error =
                Dream.sql info.ctx.request
                  (Db.insert_user ~display_name ~public_facing_id:user_id)
              in
      ...
      [
        io_field "users" ~doc:"All publicly-visible users"
          ...
          ~args:
            Arg.
              [ arg "id" ~typ:string; arg' "limit" ~typ:int ~default:(`Int 10) ]
          ~resolve:(fun (info : resolve_info_t) () id limit ->
            match id with
            | None ->
                let* users_or_error =
                  Dream.sql info.ctx.request (Db.get_users ~limit)
                in
                let* users = Caqti_lwt.or_fail users_or_error in
                ...
                in
                Lwt.return @@ Ok (PaginatedUsers.create ~cursor:"TODO" ~objs)
            | Some id' ->
                let* user_or_error =
                  Dream.sql info.ctx.request (Db.get_user ~public_facing_id:id')
                in
                let* user = Caqti_lwt.or_fail user_or_error in
                Lwt.return
                  (Ok
                     (PaginatedUsers.create ~cursor:"TODO"
                        ~objs:
                          [
                            ( user
                            |> fun { id = _; public_facing_id; display_name } ->
                              User.make ~user_id:public_facing_id ~display_name );
                          ])));
      ])
